  ...
  return (
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {dishes.map((dish) => (
        ...
        >
          {/* БАРУУН ДЭЭД БУЛАНД БАЙРЛАХ ЗАСАХ, УСТГАХ ТОВЧНУУД */}
          <div className="absolute top-3 right-3 z-10 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            {/* Засах товч */}
            <button
              onClick={() => alert(`Edit item clicked for: ${dish.name}`)} // Дараа нь Edit Modal-той холбоно
          ...
          {/* Хоолны мэдээлэл харуулах хэсэг */}
          <div className="p-4 flex flex-col flex-1 gap-2">
            ...
            {/* Орцуудыг жижиг таг хэлбэрээр харуулах */}
            ...
            {/* Категорийн нэрийг доор нь харуулах */}
            <div className="mt-auto pt-2 text-xs text-muted-foreground border-t border-gray-50">
              Category: <span className="font-medium text-gray-700">{dish.category?.categoryName || "General"}</span>
            </div>
          </div>
        </div>
      ))}
